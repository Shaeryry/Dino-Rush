...
bool GameEngine::DrawBitmap(const Bitmap* bitmapPtr, RECT destRect, RECT sourceRect) const
{
	if (m_IsPainting)
	{
		if (!bitmapPtr->Exists()) return false;
		...
		// Calculate source width and height from rect
		int srcWidth = sourceRect.right - sourceRect.left;
		int srcHeight = sourceRect.bottom - sourceRect.top;

		// Calculate destination width and height from destRect
		...
		{
			BLENDFUNCTION blender = { AC_SRC_OVER, 0, (BYTE)(2.55 * opacity), AC_SRC_ALPHA }; // blend function combines opacity and pixel-based transparency
			AlphaBlend(m_HdcDraw, destRect.left, destRect.top, destWidth, destHeight, hdcMem, sourceRect.left, sourceRect.top, srcWidth, srcHeight, blender);
		}
		else
		{
			TransparentBlt(m_HdcDraw, destRect.left, destRect.top, destWidth, destHeight, hdcMem, sourceRect.left, sourceRect.top, srcWidth, srcHeight, bitmapPtr->GetTransparencyColor());
		}
