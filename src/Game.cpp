...
void Game::Initialize()			
{
	...
	GAME_ENGINE->SetTitle(_T("Rity Kipula"));	 
	
	GAME_ENGINE->SetWidth(1024);
	GAME_ENGINE->SetHeight(768); 
    GAME_ENGINE->SetFrameRate(50);

	m_LuaHandler.Bind(GAME_ENGINE);
	m_LuaHandler.LoadScripts({ "game.lua" });

	m_LuaHandler.Initialize();
}
...
void Game::Tick()
{
	...
	const float elapsedSec{ 1.f / GAME_ENGINE->GetFrameRate() };
	m_LuaHandler.Update(elapsedSec);  
}
